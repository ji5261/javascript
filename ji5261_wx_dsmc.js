 ...
 * 微信小程序:都市牧场  为你加油
 * 支持功能：完成每天任务和自动抽奖任务
 * 完成任务，可以抽红包，当然，也有可能抽到没用的东西。
 * 注意⚠️首次抽奖需要手动去小程序授权⚠️
 * 正常登录后进入小程序首页~首页悬浮“集拼图赢大奖”，获取token值填入变量
 * https://soda.wan888888.com/v4/api/home/get-user-info?token=xxxxxxx
 *
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export dsmc_data='token@token'  多个账号用 换行 或 @分割
 * 
 */
const $ = new Env("都市牧场");
...
///////////////////////////////////////////////////////////////////
let ckStr = process.env.dsmc_data;
let msg = "";
let ck = "";
//let host = "soda.wan888888.com";
//let hostname = "https://" + host;
let ck_status = "";
///////////////////////////////////////////////////////////////////
let VersionCheck = "1.0.0"
let Version_latest = "1.0.0"
let Change = ''
let thank = `\n 感谢 蛋炒饭 大佬的开源源码 \n`
///////////////////////////////////////////////////////////////////

async function tips(ckArr) {
	//let Version_latest = await Version_Check('dsmc');
...
!(async () => {
	let ckArr = await getCks(ckStr, "dsmc_data");
	await tips(ckArr);
...
async function signIn() {
	let Option = {
		...
		body: JSON.stringify({ "type":"sign_in", "token": ck[0]})
	};
...
async function dsmc2() {
	let Option = {
		...
		body: JSON.stringify({ "type":"upload_shopping","upload_pic": "https://soda.wan888888.com/v4/uploads/banner/20220506/7b375790117a9e2e5ca9e6021209f443.jpg", "token": ck[0]})
	};
	let result = await httpPost(Option, `晒图`);
...
async function dsmc3() {
	let Option = {
		...
		body: JSON.stringify({ "type":"visit_pdd","token": ck[0]})
	};
	let result = await httpPost(Option, `浏览拼多多`);
...
async function dsmc4() {
	let Option = {
		...
		body: JSON.stringify({ "type":"visit_jd","token": ck[0]})
	};
	let result = await httpPost(Option, `浏览京东`);
...
async function cj() {
	let url = {
		url: `https://soda.wan888888.com/v4/api/puzzle/lottery?token=${ck[0]}`,
		headers: {
			'Referer' : `https://servicewechat.com/wxb3f98135f4a0579e/10/page-frame.html`,
			'Connection' : `keep-alive`,
			'Host' : `soda.wan888888.com`,
	...
	let result = await httpGet(url, `抽奖`);
